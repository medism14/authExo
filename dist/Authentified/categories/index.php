          ...
          <tbody>
            ...
            <?php 
              ...
              foreach ($results as $res)
              {
                ...
                <!-- Designation -->
                <td class="op whitespace-nowrap border-r border-gray-800 px-6 py-4">' . $res['designation'] . '</td>
                <!-- Actions -->
                <td class="op whitespace-nowrap px-6 py-4 flex justify-center space-x-10 w-full">
                  <!-- Button edit -->
                  <form method="POST" action="">
                    <button type="submit" name="edit' . $res['id_categorie'] . '" id="edit' . $res['id_categorie'] . '" class="op bg-orange-400 py-1 px-4 rounded-lg text-gray-100 hover:bg-orange-500 transition duration-200 ease-in"><i class="fas fa-edit"></i></button>
                  </form>
                    <!-- Button view -->
                  <form method="POST" action="">
                    <button type="button" id="view' . $res['id_categorie'] . '" data-modal-target="defaultModal' . $res['id_categorie'] . '" data-modal-toggle="defaultModal' . $res['id_categorie'] . '" class="op bg-blue-500 py-1 px-4 rounded-lg text-gray-800 hover:bg-blue-600 transition duration-200 ease-in"><i class="fas fa-eye"></i></button>
                  </form>
                    <!-- Button delete -->
                  <form method="POST" action="">
                    <button type="button" name="delete' . $res['id_categorie'] . '" id="delete' . $res['id_categorie'] . '" class="op cursor-pointer min-h-full bg-red-500 py-1 px-4 rounded-lg text-gray-100 hover:bg-red-600 transition duration-200 ease-in min-h-full"><i class="fas fa-trash-alt"></i></button>
                  </form>
                </td>
                  <!-- Main modal -->

                  <div id="defaultModal' . $res['id_categorie'] . '" tabindex="-1" aria-hidden="true" class="flex justify-center items-center fixed z-50 top-0 p-4 md:inset-0 h-[calc(100%-1rem)] hidden">
                      <div class="relative w-full max-w-2xl">
                          <!-- Modal content -->
                          <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                              <!-- Modal header -->
                              <div class="flex items-start justify-between p-4 border-b rounded-t dark:border-gray-600">
                                  <h3 class="text-xl font-semibold text-gray-900 dark:text-white" id="header' . $res['id_categorie'] . '">
                                      
                                  </h3>
                                  <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" id="closeModal' . $res['id_categorie'] . '"">
                              ...
                              <!-- Modal body -->
                                  ...
                                  <div class="flex">
                                    ...
                                    <div class="w-full md:w-1/2 min-h-full flex justify-start items-center p-4 text-gray-100">
                                      Designation : <span class="ml-1" id="designation' . $res['id_categorie'] . '"></span>
                                    </div>
                                    <!-- Column -->
                                    <div class="w-full md:w-1/2 p-4 text-gray-100 produits flex items-center">
                                          <div class="flex-2 mr-5">Produits :</div>
                                          <div class="flex-1 ml-1" id="produits' . $res['id_categorie'] . '" ></div>
                                    </div>
                                  </div>
                            </div>
                        </div>
